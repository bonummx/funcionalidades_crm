																		...
																		<tr><td style="padding:5px">
																				...
																				<table border=0 cellspacing=0 cellpadding=0 width=100% class="small detailview_header_table">
																					<tr style="text-align: center;">
																						<td ></td>
																						<td
																							{if $MODULE eq 'Contacts' && ($header|strstr:'complementopfn' || $header|strstr:'poderesn' || $header|strstr:'pldn' || $header|strstr:'AFEX' || $header|strstr:'Mesa de Control' || $header eq 'Origen y Destino de los Recursos' || $header eq 'Datos Personales' || $header eq 'directivos' || $header eq 'princlientesn' || $header eq 'checklistdoc' || $header eq 'Persona Fisica' || $header|strstr:'declaraciones' || $header|strstr:'consultaburo')}
																									{'style="background-color:#d61f43;color: #fff; font-size:16px"'}
																								{elseif $MODULE eq 'clientescartera' && ($header|strstr:'CobroPago' || $header|strstr:'fideicomisos' || $header|strstr:'admonarrendamiento' || $header|strstr:'Project' || $header|strstr:'Vendors' || $header|strstr:'ModComments')}
																									{'style="background-color:#d61f43;color: #fff; font-size:16px"'}
																							{/if}
																						>
																							{if $MODULE eq 'Contacts'}
																								{if $header eq 'Origen y Destino de los Recursos'}
																										{'DECLARACIONES'}
																									{elseif $header eq 'Datos Personales'}
																										{'DATOS PERSONALES'}
																									{elseif $header eq 'directivos'}
																										{'DATOS DEL CRÉDITO'}
																									{elseif $header eq 'princlientesn'}
																										{'REFERENCIAS'}
																									{elseif $header eq 'checklistdoc'}
																										{'DOCUMENTOS'}
																									{elseif $header eq 'Persona Fisica'}
																										{'INFORMACIÓN DE PERSONA FÍSICA'}
																									{elseif $header|strstr:'Mesa de Control'}
																										{'APROBACIÓN MESA DE CONTROL'}
																									{elseif $header|strstr:'AFEX'}
																										{'PRE-APROBACIÓN AFEX'}
																									{elseif $header|strstr:'complementopfn'}
																										{'COMPLEMENTOS'}
																									{elseif $header|strstr:'poderesn'}
																										{'INFORMACIÓN PARA EL ÁREA LEGAL'}
																									{elseif $header|strstr:'pldn'}
																										{'INFORMACIÓN PARA PLD'}
																									{elseif $header|strstr:'pldn'}
																										{'INFORMACIÓN PARA PLD'}
																									{elseif $header|strstr:'declaraciones'}
																										{'INFORMACIÓN PARA ANÁLISIS DE CRÉDITO'}
																									{elseif $header|strstr:'consultaburo'}
																										{'REPORTE DE BURÓ DE CREDITO'}
																								{/if}
																								{elseif $MODULE eq 'clientescartera'}
																									{if $header eq 'CobroPago'}
																										{'LÍNEA DE CRÉDITO'}
																									{elseif $header|strstr:'fideicomisos'}
																										{'FIDEICOMISOS'}
																									{elseif $header|strstr:'admonarrendamiento'}
																										{'ARRENDAMIENTO'}
																									{elseif $header|strstr:'Project'}
																										{'CREDITOS'}
																									{elseif $header|strstr:'Vendors'}
																										{'FACTORAJE'}
																									{elseif $header|strstr:'ModComments'}
																										{'COBRANZA'}
																									{/if}
																							{/if}</td>
																						<td></td>
