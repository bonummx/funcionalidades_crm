...
class vtigerCRM_Smarty extends Smarty {
	...
	public function __construct() {
		...
		$this->assign('CURRENT_USER_ROLEID', (isset($current_user) ? $current_user->roleid : 0));
		$this->assign('Application_JSCalendar_Load', GlobalVariable::getVariable('Application_JSCalendar_Load', 1, $currentModule));
		// Se agrego para que podamos cerrar funciones externas cuando la sesion no exista
		coreBOS_Session::init(true, true);
		coreBOS_Session::set('authenticated_user_roleid', (isset($current_user) ? $current_user->roleid : 0));
